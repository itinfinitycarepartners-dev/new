//@ts-nocheck
// Login.jsx - Fixed: correctly routes new/unverified candidates to password setup
// (bypassing OTP) instead of showing "contact support" for hasPassword === false.
...
export default function Login() {
  ...
  const handleCheckEmail = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // That must route to password setup, not to an error message.
        if (data.needsPasswordSetup) {
          setEmail(enteredEmail);
          setStep("setup-password");
          setInfo(data.message || "Email confirmed. Please create your password.");
          ...
          // Fallback: only reachable if the backend explicitly signals the
          // account exists but truly cannot be set up (no OTP, no password
          // setup path offered).
          setError(
            data.message ||
  ...
  const handlePasswordLogin = async (e) => {
    ...
    try {
      ...
      // The candidate login-with-password endpoint can also report that the
      // account needs password setup (e.g. it was verified via check-email
      // on another device/session but never completed). Route accordingly
      // instead of just treating it as a generic failure.
      if (data.needsPasswordSetup) {
        setStep("setup-password");
        setInfo(data.message || "Please create your password.");
        return;
      }
